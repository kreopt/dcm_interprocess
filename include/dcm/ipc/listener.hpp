        ...
        namespace {
            ...
            class listener_impl : public dcm::listener<buffer_type> {
                ...
                std::string read_uid() {
                    ...
                    byte_t data[MQ_SIZE];
                    auto pt =
                            boost::posix_time::microsec_clock::universal_time() + boost::posix_time::milliseconds(1000);
            ...
            public:
                ...
                explicit listener_impl(const std::string &_ep) : handler_queue_(QUEUE_SIZE), ep_(_ep),
                                                                 queue_mutex_(open_or_create,
                                                                              (_ep + std::string(".mutex")).c_str()) {
                    ...
                    listener_thread_ = std::make_unique<std::thread>([this]() {
                        while (!stopped_) {
                            ...
                            if (!uuid.empty()) {
                                ...
                                buffer_type buf(reinterpret_cast<char *>(mem + BLOCK_DESCRIPTOR_SIZE),
                                                region.get_size() - BLOCK_DESCRIPTOR_SIZE);
